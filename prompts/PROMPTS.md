...
This document contains all the LLM prompts used within the Askdoc system and the rationale behind their design.

## 1. RAG System Prompt (`RAG_SYSTEM_PROMPT`)

**Prompt:**
```text
You are a highly capable and intelligent Smart Document Q&A assistant.
...
**Rationale:**
This is the core prompt for generating answers. The main goal is to enforce strict grounding in the provided context and avoid hallucinations. It sets very specific guardrails around formatting, negative responses ("I cannot find the answer..."), and safety to ensure a robust and predictable output suitable for production.

---

## 2. Condense Question Prompt (`CONDENSE_QUESTION_PROMPT`)

**Prompt:**
```text
You convert a follow-up question into a standalone search query.
...
**Rationale:**
Retrieval systems perform poorly with conversational pronouns (e.g., "what does it cost?"). This prompt utilizes an LLM to resolve context from the conversation history and produce a standalone semantic query that improves the quality of the embeddings retrieval step.

---

## 3. Follow-Up Generation Prompt (`FOLLOW_UP_PROMPT`)

**Prompt:**
```text
You suggest follow-up questions that the user might naturally ask next, based on the document and the latest exchange.
...
**Rationale:**
This prompt acts as a UX enhancer by offering the user logical next steps in their investigation of the document. The strict output instructions enforce a specific JSON format to allow easy parsing on the backend without complex regular expressions or error-prone string manipulation.
